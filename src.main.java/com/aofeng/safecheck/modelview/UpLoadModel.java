				...
				Thread th = new Thread(new Runnable() {	
					...
					private boolean UploadFile(String path) {
						...
						try
						{
							...
							String url = Vault.files_url + "savefile" + "?FileName="
					...
					try {
						...
						if (c.moveToNext()) {
							for (String col : c.getColumnNames())
							{
								...
								obj.put(col.toUpperCase(), value==null? "": value);
							}
							c = db.rawQuery("select * from T_IC_SAFECHECK_HIDDEN where id=? order by EQUIPMENT",  new String[] { uuid });
							int sn = 1;
							while(c.moveToNext())
							{
								if(sn > 99)
									sn = 1;
								String EQUIPMENT = c.getString(c.getColumnIndex("EQUIPMENT")) + "_" + sn;
								obj.put(EQUIPMENT, c.getString(c.getColumnIndex("CONTENT")));
								sn += 1;
							}
						}
						db.close();
